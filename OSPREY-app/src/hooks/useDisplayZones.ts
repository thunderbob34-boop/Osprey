...
async function fetchDisplayZones(userId: string): Promise<DisplayZones | null> {
  ...
  if (sport === 'lift') return null; // strength has no pace zones — no card
  ...
  return { zones, hrZones, confidence };
}
...
export function useDisplayZones(): DisplayZones | null {
  ...
  const { data } = useQuery({
    queryKey: ['display-zones', userId],
    queryFn: () => fetchDisplayZones(userId!),
