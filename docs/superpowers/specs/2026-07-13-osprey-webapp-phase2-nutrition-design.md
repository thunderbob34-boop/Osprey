...
Logging a serving inserts **one row into the existing `food_log_entries` table** carrying the computed per-serving macros, exactly like logging any single food. The entry's `food_item_id` points at a lightweight auto-created `food_items` row (`source = 'recipe'`, one per recipe, tracked via `recipes.shadow_food_item_id`) — created on first log, name/macros refreshed on subsequent logs if the recipe changed — so the FK stays valid and the entry renders under the recipe's name.
