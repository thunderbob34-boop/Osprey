...
5. **Coaching showcase** — h2 "ONE COACH. / EVERY SESSION." Full-width 4-cell tab bar (RUN / STRENGTH / NUTRITION / COACH); active tab is a solid amber block with black text. Below, a hard-bordered device frame (`--panel`) with a top bar (OSPREY / mode label) and a body: uppercase context label → oversized metric → Ozzie coach line (amber name, warm voice). Tab click swaps frame content; each tab has its own scene (run pace, strength set, protein target, deload brief). Content swap is instant or ≤150ms fade — nothing springy.
