...
- **Do NOT read `goal_params`** in the chat function's context builder — that column is in the pending-undeployed bundle, and reading it would make chat depend on that deploy (the exact trap the home dashboard's final review caught). Read only already-deployed objects.
...
```
plus `ENABLE ROW LEVEL SECURITY`, `ozzie_conversations_self` / `ozzie_messages_self` policies (`USING (user_id = auth.uid())`), and `GRANT SELECT, INSERT, UPDATE, DELETE ON <each> TO authenticated`.
...
`POST { conversationId: string, message: string, todayISO: string }` → `text/event-stream`.

1. **Auth** — `Authorization` header → `supabase.auth.getUser(token)`; 401 without a valid user. Build the Supabase client with the caller's token so every read/write is RLS-scoped to them.
2. **Verify the thread is theirs** — `SELECT id FROM ozzie_conversations WHERE id = conversationId` under the caller's token; absent → 404. **This check is load-bearing:** `ozzie_messages`'s RLS policy is `user_id = auth.uid()`, which authorises *the row's owner* but says nothing about *which conversation the row lands in* — so RLS alone would let a caller append to a thread they don't own. Nothing leaks either way (reads are equally `user_id`-scoped, so neither party sees the other's rows), but the function must not write a row it cannot justify.
...
4. **Build context** — already-deployed objects only, each bounded so the prompt cannot grow without limit:
   ...
   - `training_sessions` for the current week — Mon–Sun derived from the client-sent `todayISO`. The function has no local clock and must not guess the athlete's timezone; the webapp already computes its local date with `toDateInputValue` (`webapp/src/lib/day.ts`) and sends it. A client sending a wrong date only mis-scopes its own context.
...
- **`webapp/src/features/chat/send.ts`** — the streaming call. NOT React Query (it's a stream): POST to the function with the session's access token and the local `todayISO`, read the `ReadableStream`, decode SSE chunks, and hand each token to a callback. On completion, invalidate `['messages', conversationId]` + `['conversations', userId]` (the function already persisted both turns).
...
- `context_test.ts`, `stream_test.ts` — the pure-builder tests.
...
5. The chat function returns 401 without a valid JWT, and never selects `goal_params`.
